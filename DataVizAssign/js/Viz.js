  ...
  var selectedYear = 0, selectedIndex = 0, selectedtennisball, position, isPause = false;
  ...
  d3.select('button').on('click', function() {
    if(d3.select('button').html() == "Resume") {
      d3.select('button').html("Pause");
      isPause = false;
      showTransition(position+1);
    } else if(d3.select('button').html() == "Pause") {
      d3.select('button').html("Resume");
      isPause = true;
    }
  });
  ...
  function showTransition(i){
    ...
    if(responsestruct[i].round != ""  && isPause == false) {
      position = i;
      if(responsestruct[i].result == "win"){
          ...
          pauseButton = document.getElementById('pauseButton').hidden = false;
    ...
    if(i<6 && responsestruct[i].round != "" && isPause == false) {
  ...
  function drawHistogram() {
    /*
    // Create histogram and draw its axes
    var h1 = d3.select("body").append("svg").attr("id","svgHist1").attr("height", "300px").attr("width", "500px").attr("role","histogram");
    var hsvg = d3.select('[role="histogram"]');
    var histogram = hsvg.attr('width',500).attr('height',300).append('g');

    var x0 = d3.scale.ordinal().rangeRoundBands([0, 500], .1);

    var x1 = d3.scale.ordinal();
    var color = d3.scale.ordinal().range(["#98abc5", "#a6898a", "#887b68"]);
    var histXAxisScale = d3.scale.linear().range([0,330]).domain([4,14]),
    ...
    histogram.selectAll('rect')
      ...
      .append('rect')
      .attr('fill','red')
      .attr('height',function(d) { return Number(d.f1);})
      .attr('width',20)
      .attr('x',function(d, i) {
        return (margin.left*2) + (i*20);
      });
      .append('rect')
      .attr('fill','green')
      .attr('height',function(d) { return Number(d.n1);})
      .attr('width',20)
      .attr('x',function(d, i) {
        return (margin.left*2) + (i*40);
      })
      .append('rect')
      .attr('fill','blue')
      .attr('height',function(d) { return Number(d.r1);})
      .attr('width',20)
      .attr('x',function(d, i) {
